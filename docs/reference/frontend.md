...
| `/documents/[id]` | `documents/[id]/+page.svelte` | Document detail: PDF viewer (self-hosted pdf.js — zoom, fit-width, find-in-PDF, selectable text, lazy rendering), **Content Stats** (pages, words, characters). Editable metadata sidebar (title, type, language, tags, people). Controls hidden for viewer role. |
...
- **`src/lib/components/PdfViewer.svelte`** — Self-hosted PDF viewer (replaces the bare `<iframe>` embed on the document detail page). Loads the document via `getDocument({ url })` (same-origin fetch, `edub_token` cookie auth), icon toolbar (via `Icon.svelte`) with zoom out / fit-width / zoom in (±0.25 steps, clamped [0.5, 4], Ctrl+wheel zoom), a jump-to-page input (`n / numPages`, Enter jumps, clamped to [1, numPages], synced with scroll) with prev/next chevrons, and a find-in-PDF toggle (search icon; the find bar — debounced query input, match count, prev/next navigation, span highlighting via `pdf-find-highlight`/`pdf-find-current` classes — renders inline in the toolbar row). Navigation scrolls are scoped to the viewer's own scroll container (`scrollEl.scrollTo` with cached `pageOffsets`; match spans centered via `scrollSpanIntoView`) so the page-level scroll (`<main>`) never moves. Continuous vertical scroll with lazy rendering: dual IntersectionObserver (render within ~2 viewports, unrender beyond ~6, `renderZone`/`keepRendered` SvelteSet hysteresis reconciled per microtask so observer delivery order cannot leak rendered pages), page proxies fetched lazily on observer entry (page 1 eager for fit-scale + placeholder aspect ratio), `getTextContent` cached per page for find. Loading spinner and error state with download link (`?download=true`).
- **`src/lib/components/PdfPage.svelte`** — Per-page canvas + text layer. Canvas is sized declaratively in markup (`width`/`height` attributes from the viewport, `role="img"` + `aria-label="Page N"`, hidden while inactive) and rendered at `viewport × devicePixelRatio` (capped 2×) via `pdfPage.render()`; a `TextLayer` (`streamTextContent({ includeMarkedContent, disableNormalization })`) provides selectable text. The text-layer container div is `{#if active}{#key scale-docKey}`-keyed so Svelte owns its lifecycle (no manual DOM clearing); `docKey` bumps on page-proxy identity change so doc reloads get a fresh container. Registers an imperative API with the parent (`textDivs`, `applyHighlights`, `markCurrent`) for find highlighting. In-flight canvas tasks and text-layer streams are cancelled on deactivation/zoom.
- **`src/lib/components/Icon.svelte`** — Inline SVG icon component (lucide-style, `stroke="currentColor"`, `aria-hidden="true"`): named icons `minus`, `plus`, `chevron-left`, `chevron-right`, `search`; optional `size` prop (default 16). Used by the viewer toolbar; avoids `{@html}` string injection.
- **`src/lib/pdf/pdfjs.js`** — pdf.js loader singleton: imports `pdfjs-dist`, sets `GlobalWorkerOptions.workerSrc` from the Vite-emitted worker bundle (`pdf.worker.js?worker&url`), re-exports `getDocument` and `TextLayer`.
- **`src/lib/pdf/pdf.worker.js`** — Worker wrapper: applies the `Promise.withResolvers` polyfill (from `polyfills.js`) in the worker realm, then dynamically imports the real `pdf.worker.min.mjs` asset (also polyfills `AbortSignal.any`/`crypto.randomUUID` on the main thread via `app.html`).
- **`src/lib/pdf/polyfills.js`** — The `Promise.withResolvers` polyfill, shared by the worker wrapper (the `app.html` inline copy additionally covers `AbortSignal.any` and `crypto.randomUUID`).
