...
| `/documents/[id]` | `documents/[id]/+page.svelte` | Document detail: PDF iframe + DOCX/ODT client-side preview via OfficePreview component (officeparser — formatted HTML with scoped CSS), **Content Stats** (pages, words, characters). Editable metadata sidebar (title, type, language, tags, people). MIME-aware download button label. Controls hidden for viewer role. |
...
- **`src/lib/components/OfficePreview.svelte`** — DOCX/ODT client-side preview component. Fetches the file from `/api/v1/documents/{id}/file`, parses it with the `officeparser` library (lazy-loaded via dynamic `import()`), and renders formatted HTML with scoped CSS (`@scope` rules). Handles loading, error (with download fallback), empty, and success states. Enforces a 20 MB file size guard before parsing.
