...
| `/documents`      | `documents/+page.svelte`      | Document list with structured search bar, filter panel, saved searches, sort/pagination; shows **Tags** and **People** columns inline; snippet column when searching. Batch delete, batch tag assign, and batch set-type buttons hidden for viewer role. Saved search Save/Delete hidden for viewer. |
