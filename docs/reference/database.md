...
- `WaitForTaskDrain(ctx, queries, logger, what)` — Polls `CountProcessingTasks` (5s ticker) until no consume/enrich task is processing; the caller must hold the backup lock, which blocks new claims, so it converges. Shared by `BackupTaskHandler` and the `migrate-db` / `migrate-storage` handlers.
