...
- `ScanAndEnqueue(ctx, cfg, client, logger) (batchID string, enqueued int, err error)` — Shared inbox scan → dedup → batch creation function. Scans the consumption directory via `utils.ListFilePaths`, computes MD5 for each file, batch-deduplicates against existing documents via `queryDuplicatesByMD5`, and creates consume+enrich task pairs. Scan-time duplicates are **moved** to `storage/errors/duplicated/` (via `MoveFailedFile(..., "duplicate", ...)`) instead of being skipped in place: a file whose MD5 matches an existing document is only quarantined when its SHA512 also matches the stored `sha512_checksum` (an MD5 collision with a differing SHA512 is enqueued normally), and a second copy of a file already enqueued earlier in the same scan is moved too (guarded by the `consume:<md5>` dedup key). The batch record is created **after** all tasks are committed (with status `queued`) to prevent the queue consumer from picking up an incomplete or empty batch. Returns an empty batch ID when no files or all duplicates are found.
- `queryDuplicatesByMD5(ctx, client, hashes) (map[string]duplicateInfo, error)` — Single SQL query (`SELECT md5_checksum, document_id, sha512_checksum FROM document WHERE md5_checksum IN (...) AND deleted_at IS NULL`) for all MD5 hashes, returns a map keyed by MD5 → `duplicateInfo{documentID, sha512}` for O(1) lookups in the dedup loop. The `deleted_at IS NULL` filter keeps parity with the consume-time checksum lookups (`GetDocumentByMD5Checksum`/`GetDocumentBySHA512Checksum`) so soft-deleted (trashed) documents never trigger quarantine.
  ...
  - **Methods**:
    - `NewEnricher(cfg, logger, queries, services, matcher tagmatcher.Matcher) (*Enricher, error)` — Creates runner via `NewRunnerWithMatcher` with textreducer, contentanalyzer, tagmatcher tools; matcher is either a direct `*Hugot` (in `kushim` CLI) or a `*tagmatch.MatcherClient` (in `edub` API server) — both satisfy the same `tagmatcher.Matcher` interface.
     - `Enrich(ctx, document) (*json.RawMessage, error)` — Full pipeline:
       ...
       7. **Tag normalization** — LLM-extracted tags are run through `NormalizeTags` (delegates to `utils.NormalizeTag`: NFKC, lowercase, dash family → space, accent folding, keeps letters of any Unicode script, digits, and `+ # .`, strips everything else, collapses whitespace), then deduplicated and empties rejected
       ...
       9. Post-LLM tag consolidation via `Runner.RankTags` (matcher returns ranked candidates with scores) + `applyConsolidationPolicy` → `tagpolicy.Evaluate` (ordered gates: identity, negation, shared-token, direction; then a guard-free candidate at or above `auto_replace_similarity` replaces; the ambiguous band is resolved by LLM adjudication when `enricher.tag_adjudicator.enabled` is true, otherwise keeps)
