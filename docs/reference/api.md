...
- `ConfigHandler`
  ...
  - **Methods**:
    ...
    - `PutConfig(w, r)` — `PUT /wizard/config` — Three-phase: if `config_dir` is present and no config exists, bootstraps config directory, DB, and skeleton YAML. Otherwise, when the body changes any `database.*` connection setting (`dbParamsFromBody` → `config.DatabaseConnectionChanged`, DSN-aware), it persists non-DB/non-storage keys immediately, enqueues a `migrate-db` config task with the new connection settings, and returns `202 {"pending_tasks":1}` — the settings apply only after the data copy completes (a task in flight answers `409`, a stale pending/failed task is superseded atomically via `DeleteConfigTaskByDedupKey`). For any other save it writes config via `SaveMap`, reloads, and enqueues config tasks for missing downloads (tessdata, hugot). Returns `200` or `201` with pending task count and a `missing_tools` array of hard-blocking tool-availability issues. Requires admin role.
