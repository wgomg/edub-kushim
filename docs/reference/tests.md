...
| `internal/database` | 38 | sqlc-generated CRUD, task lifecycle, enrich waiting flow, enrich discard/restore/sweep queries (SetEnrichTaskWaiting, RestoreDiscardedEnrichTasks, DiscardWaitingEnrichesOfFailedConsumes), batch ownership, FTS-adjacent operations, document/tag/people/document-type CRUD, saved searches, dashboard analytics queries (empty DB + mixed data), structured search missing filters (MissingLanguage/MissingType/Untagged), structured search people filters (typed relationship scoping, `PersonAnyType` across relationship types, unknown-person miss, count/search parity), structured search sort by `page_count` (asc/desc ordering, unknown sort key falls back to default), WithDocumentCount queries, backup lock lifecycle, gated task claiming, soft delete (TestUpdateDeleteDocument now asserts soft-delete semantics), CheckRestoreTooling (psql/docker/podman present, missing binary, missing container, remote-block, invalid runtime) |
...
| `internal/api/handlers` | 72 | Document CRUD, tag/people/DocumentType CRUD, user CRUD (with role), task endpoints, saved searches, concurrent operations, dashboard running tasks + analytics + processing health, analytics error path, config handler get/status, PutConfig runtime/container immediate-persistence alongside db migration, batch delete limits, error helpers, auth login (valid/invalid/empty/claims/role), auth logout, API key generate/revoke/rotate/status/forbidden/invalid-id/not-found + wire-format contract (has_api_key present, has_key absent, api_key_prefix present), MeHandler (valid/missing-id/not-found), self-service API key handlers (MeGenerateKey/MeRevokeKey/MeRotateKey/MeGetKeyStatus/unauthorized + wire-format contract), orphaned handler (list/scan/delete/restore/move-to-inbox/delete-all/move-all), errored handler (list/download/delete/delete-all), logs handler (invalid name/file not found/success/line clamping/large file tail/empty file/oversized line truncation/no trailing newline), people-type reserved-name guard (create/update rejected for `person`, case-insensitive; non-reserved name accepted) |
...
environment. The SQL-dump restore tests execute `psql` through the `podman`
runtime against the `edub-test-pg` container (see the AGENTS.md test-database
instructions) instead of requiring `psql` on the host PATH.
