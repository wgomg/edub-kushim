...
| `internal/database` | 36 | sqlc-generated CRUD, task lifecycle, enrich waiting flow, enrich discard/restore/sweep queries (SetEnrichTaskWaiting, RestoreDiscardedEnrichTasks, DiscardWaitingEnrichesOfFailedConsumes), batch ownership, FTS-adjacent operations, document/tag/people/document-type CRUD, saved searches, dashboard analytics queries (empty DB + mixed data), structured search missing filters (MissingLanguage/MissingType/Untagged), structured search people filters (typed relationship scoping, `PersonAnyType` across relationship types, unknown-person miss, count/search parity), WithDocumentCount queries, backup lock lifecycle, gated task claiming, soft delete (TestUpdateDeleteDocument now asserts soft-delete semantics) |
