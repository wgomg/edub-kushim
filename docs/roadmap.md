...
| 41  | **Cap retry count on batch task reset**                | `ResetProcessingTasksByBatch` currently resets processing tasks to pending without limit. A per-task attempt counter that marks a task `failed`/quarantined after N consecutive failures bounds the worst case even for files that crash on 100% of attempts. See [report](.kilo/reports/2026-07-02-mupdf-uaf-pdfclean-crash-loop.md).                                                                                                                                                                                                   |
| 42  | **Signal orphaned children in stale reclamation**      | `reclaimStaleBatches` should signal (SIGTERM, then SIGKILL) the previous owner's PID before deleting its `batch_owner` row, matching `consumeCancelHandler`'s existing pattern. This bounds resource pileup for any future cause of staleness. See [report](.kilo/reports/2026-07-02-mupdf-uaf-pdfclean-crash-loop.md).                                                                                                                                                                                                                  |
...
| 46  | **API keys**                                           | Key generation/revocation/rotation, Bearer token validation middleware, per-user key management                                                                                                                                                                                                                                                                                                                                                                                                                                          |
