...
| 32  | **Batch queue schema and queries**                     | Migration, updating existing batches, adding new queries and batch service methods                                                                                                                                                                                                                                                                                                                     |
| 33  | **`kushim queue` daemon**                              | New daemon CLI command (sibling to `kushim hugot`). Runs two loops: (a) queue consumption — `GetNextQueuedBatch`, fork `kushim consume --batch <id>` when `CountLiveBatches < MaxConcurrentBatches`; (b) stale reclamation — `ListStaleBatchOwners` (>15s heartbeat with active tasks), reset processing→pending, delete stale owner, set batch→`queued`. `--bg` to daemonize. PID file in config dir. |
| 34  | **Move inbox polling to `kushim queue`**               | Remove `PollingScheduler` from edub (`internal/scheduler/`). Embed inbox scanning loop in `kushim queue` as a third loop. Before enqueueing: check `CountQueuedBatches + CountLiveBatches < MaxConcurrentBatches`. Respects existing `consumer.polling` config (interval, windows, enabled).                                                                                                           |
...
| 39  | **Orphan Restore creates proper batch**                | `Restore()` creates batch record with `status='queued'`, creates matched consume+enrich task pair (fixes missing enrich task bug). No direct forking — queue daemon picks it up. `MoveToInbox` path unchanged.                                                                                                                                                                                         |
| 40  | **Setup creates initial admin user**                   | `kushim setup` (CLI + wizard) prompts for admin username + password before completion, creates user via `service.User.Create()`. Session secret auto-generated and persisted during setup.                                                                                                                                                                                                             |
| 41  | **API keys**                                           | Key generation/revocation/rotation, Bearer token validation middleware, per-user key management                                                                                                                                                                                                                                                                                                        |
| 42  | **Roles and permissions**                              | RBAC schema with admin/editor/viewer roles, permission enforcement middleware, role assignment                                                                                                                                                                                                                                                                                                         |
| 43  | **Document notes/comments**                            | User-added notes and annotations on documents                                                                                                                                                                                                                                                                                                                                                          |
| 44  | **Pre-built binaries**                                 | Release binaries for major platforms (Linux amd64/arm64)                                                                                                                                                                                                                                                                                                                                               |
| 45  | **MySQL / MariaDB database backend**                   | Additional database backend support                                                                                                                                                                                                                                                                                                                                                                    |
| 46  | **User preferences**                                   | Theme, pagination defaults, notification settings                                                                                                                                                                                                                                                                                                                                                      |
| 47  | **Email ingestion**                                    | IMAP inbox scanning                                                                                                                                                                                                                                                                                                                                                                                    |
| 48  | **Document relationships**                             | Parent/child, cross-references between documents                                                                                                                                                                                                                                                                                                                                                       |
| 49  | **Contributing guide & issue templates**               | `CONTRIBUTING.md`, issue/PR templates, and GitHub community setup to encourage contributions                                                                                                                                                                                                                                                                                                           |
