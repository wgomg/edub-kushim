...
| 47  | **Roles and permissions**                              | ✓ RBAC schema with admin/editor/viewer roles, permission enforcement middleware, role assignment, self-service `/me` endpoints. See [plan](/.kilo/plans/1783225275085-rbac-roles-permissions.md)                                                                                                                                                                                                                                                                                                                                         |
| 48  | **Role-based UI**                                      | ✓ Show/hide UI elements by role (admin-only: Users page, Config page, Logs; editor-only: Upload, batch actions, delete buttons; viewer: read-only). Role badge in user list. Role dropdown in user create/edit forms. Self-service profile page with API key management and role display. Frontend reads role from `GET /api/v1/me`. See [plan](/.kilo/plans/1783227785956-role-based-ui.md).                                                                                                                                                                                                       |
...
| 50  | **Document notes/comments**                            | User-added notes and annotations on documents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 51  | **Native structured logging via slog**                 | Migrate from the adapter wrapper (`utils.Logger` with `reqID *string` + Printf) to native `slog.Logger` interfaces. Replace `reqID *string` with context-bound loggers via `slog.Logger.With()`, add `InfoContext`/`ErrorContext`/`WarnContext`/`DebugContext` throughout. Enables fully structured key-value attributes at every call site (instead of format strings), direct integration with log aggregators (Loki/ELK), and eliminates ~257 `reqID` parameter references across the codebase.                                       |
| 52  | **Pre-built binaries**                                 | Release binaries for major platforms (Linux amd64/arm64)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 53  | **MySQL / MariaDB database backend**                   | Additional database backend support                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 54  | **User preferences**                                   | Theme, pagination defaults, notification settings                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 55  | **Email ingestion**                                    | IMAP inbox scanning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 56  | **Document relationships**                             | Parent/child, cross-references between documents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 57  | **Contributing guide & issue templates**               | `CONTRIBUTING.md`, issue/PR templates, and GitHub community setup to encourage contributions                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 58  | **Research: LLM provider capability discovery**        | Investigate how each supported provider (OpenAI, Anthropic, DeepSeek, Mistral/Ollama) exposes model capabilities — available models, supported request parameters, allowed enum values per parameter, and how they differ from the OpenAI standard. Map the compatibility surface to inform adapter design and avoid 400 errors from unsupported fields.                                                                                                             |
| 59  | **Limit language distribution to top 10 in dashboard** | The language distribution section in the document analytics panel (Dashboard) currently shows every detected language. Cap it at the 10 most frequent languages, matching the existing top-N limit for tags. Prevents the list from being dominated by one-off languages in large collections.                                                                                                             |
| 60  | **Fix "Cumulative Storage Trend" graph width**         | The cumulative storage trend chart in the storage panel (Dashboard) does not use the full available width, leaving an empty gap on the right side. Fix the SVG/container sizing so the graph fills its allocated space consistently across viewport sizes.                                                                                                                                    |
...
| 62  | **Expose "warn" log level in settings UI**             | A "warn" log level was recently added to the backend's structured logging. The web UI settings page log level selector currently offers only "debug", "info", and "error". Add "warn" as a selectable option so users can configure the new level through the UI.                                                                                                                             |
