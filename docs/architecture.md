...
A long-lived daemon that manages batch processing and inbox polling. Batch consumption is driven by Postgres `LISTEN`/`NOTIFY` (via a dedicated pgxpool connection), with a 30-second safety timer as fallback. Housekeeping tasks (stale reclamation, backup scheduling, mirror scheduling, scheduled thumbnail backfill, empty-queued-batch sweep) run on a separate 5-second ticker. The concurrent loops are: (a) **notification-driven consumption** — when any batch transitions to `status='queued'`, a Postgres trigger (`notify_batch_queued`) sends a `pg_notify` to the `batch_queued` channel; a dedicated `LISTEN` goroutine forwards the signal over a buffered channel, the daemon picks up the batch immediately (milliseconds) and forks a worker. (b) **safety timer** — a 30-second fallback poll in case a notification is dropped (connection blip, reconnection), guaranteeing no batch is stuck longer than 30s. (c) **stale batch reclamation** — reclaims stale batch owners (>15s heartbeat with active tasks) by signaling SIGTERM to the owner PID (if alive), quarantining tasks at or above `consumer.reclaim.max_retries` (default 3) to `failed`, resetting remaining processing→pending with an incremented attempt counter, calling `QuarantineFailedFiles` to move quarantined inbox files to `storage/errors/` and discard orphaned enrich tasks, then checking `HasPendingWork`. If the batch has remaining pending/processing/waiting tasks it is re-queued; otherwise (all tasks quarantined) it is set to `failed`. The stale owner row is then deleted (gated by `consumer.reclaim.enabled`, default `true`). (d) **stale task reclamation** — an age-based sweep that resets individual `processing` tasks back to `pending` (or `failed` if at max retries) when their `started_at` timestamp exceeds `consumer.reclaim.stale_task_after` (default 600s). This is the safety net for tasks the runner could not complete: handler panics are failed immediately by the runner's per-task recover, so the sweep covers hard crashes (kill -9, OOM) where no Go-level recover can run. The sweep runs at most once per `max(60s, stale_task_after/10)` to avoid unnecessary write-lock pressure. (e) **inbox polling** — periodically scans the consumption directory, deduplicates by MD5, creates `queued` batches, and lets the consumer loop pick them up. (f) **empty queued batch sweep** — marks `failed` any `queued` batch older than 10 minutes with zero tasks (`ListEmptyQueuedBatches`); such batches are never finalized by a handler and would otherwise block the polling gate capacity check forever. When `backup.enabled` is `true`, a **backup pool** (1 worker, 60s interval) picks up scheduled backup tasks; when `mirror.enabled` is `true`, a **mirror pool** (1 worker, 60s interval) picks up scheduled mirror tasks. The main ticker and polling loop both use `backupMu.TryRLock()` to skip all operations while a backup or mirror is running, preventing concurrent data access. Re-reads config from disk on each poll tick so `consumer.polling` changes take effect without restart. On the 5s housekeeping ticker, if the database connection settings changed (after a `migrate-db` task), the daemon probes the new connection (open + `InitializeSchema`), stops the backup pool, invalidates the cached DB handle, reconnects all services, and restarts the `LISTEN`/`NOTIFY` session; a failed probe keeps the previous configuration. Uses a PID file for single-instance enforcement and logs to `<config_dir>/logs/queue.log`. Can be daemonized via `--bg`.
...
mirror enabled/disabled, destination path (local or `[user@]host:path`), interval, preferred time.
