...
flowchart TB
    ...
    DB -->|"NOTIFY"| Q
    Q -->|"fork"| Worker
    EP -.->|"encode/match/rank/consolidate"| H
```
...
- **Post-LLM consolidation** — normalized tags ranked against canonical embeddings; the enricher applies a guard-based policy (negation, shared-token, direction) and only replaces a tag with a guard-free candidate at or above the auto-replace cutoff. The ambiguous band can be resolved by an optional LLM adjudicator (`enricher.tag_adjudicator`, disabled by default) that classifies pairs as `same`/`variant`/`related` with verdicts cached across documents
