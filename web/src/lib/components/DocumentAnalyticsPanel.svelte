...
<div class="space-y-6">
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
		<div class="rounded-lg border border-clay-800 bg-clay-900 p-4">
			...
			<p class="mt-1 text-lg font-semibold text-parchment-200">{formatNumber(analytics?.missing_language_count ?? 0)}</p>
		</div>
		<div class="rounded-lg border border-clay-800 bg-clay-900 p-4">
			<p class="text-sm text-parchment-500">Missing Type</p>
			<p class="mt-1 text-lg font-semibold text-parchment-200">{formatNumber(analytics?.missing_type_count ?? 0)}</p>
		</div>
		<div class="rounded-lg border border-clay-800 bg-clay-900 p-4">
			<p class="text-sm text-parchment-500">Untagged Documents</p>
			<p class="mt-1 text-lg font-semibold text-parchment-200">{formatNumber(analytics?.missing_tags_count ?? 0)}</p>
		</div>
	</div>
						...
						<div class="flex items-center gap-3">
							...
							<div class="flex-1">
								<div class="flex justify-between text-sm">
									...
									<span class="shrink-0 text-parchment-500">{formatNumber(item.count)}</span>
								</div>
								<div class="mt-1 h-2 w-full rounded-full bg-clay-800">
									<div
										class="h-2 rounded-full transition-all"
										style="width: {(item.count / maxLangCount()) * 100}%; background-color: {chartColors[i % chartColors.length]}"
									></div>
								</div>
							</div>
						</div>
						...
						<div class="flex items-center gap-3">
							...
							<div class="flex-1">
								...
								<div class="mt-1 h-2 w-full rounded-full bg-clay-800">
									<div
										class="h-2 rounded-full transition-all"
										style="width: {(item.count / maxDocTypeCount()) * 100}%; background-color: {chartColors[i % chartColors.length]}"
									></div>
								</div>
							</div>
						</div>
					...
					<table class="w-full text-sm">
						<thead>
							<tr class="border-b border-clay-800 text-left text-parchment-500">
								<th class="pb-2 pr-4 font-medium">Tag</th>
									...
									<td class="py-2 text-parchment-400">
										<div class="flex items-center gap-2">
											<div class="h-2 w-24 overflow-hidden rounded-full bg-clay-800 sm:w-32">
												<div
													class="h-2 rounded-full transition-all"
													style="width: {(item.count / maxTagCount()) * 100}%; background-color: {chartColors[i % chartColors.length]}"
												></div>
											</div>
