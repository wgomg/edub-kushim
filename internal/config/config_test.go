...
func TestLoad_Defaults(t *testing.T) {
	...
	if cfg.Consumer.TextExtractor != "mupdf" {
		t.Errorf("Consumer.TextExtractor = %q, want mupdf", cfg.Consumer.TextExtractor)
	}
...
func TestLoad_MissingOCRLanguages(t *testing.T) {
	...
	_, err := Load(dir)
	if err == nil {
		t.Fatal("expected error for missing ocr_languages")
	}
}
...
`), 0644)
	...
	if len(cfg.Consumer.OCRLanguages) != 2 {
		t.Fatalf("OCRLanguages len = %d, want 2", len(cfg.Consumer.OCRLanguages))
	}
	if cfg.Consumer.OCRLanguages[0] != "eng" || cfg.Consumer.OCRLanguages[1] != "spa" {
		t.Errorf("OCRLanguages = %v", cfg.Consumer.OCRLanguages)
	}
...
func TestLoad_TildeExpansion(t *testing.T) {
	...
	if cfg.Consumer.OCRDataDir != want {
		t.Errorf("OCRDataDir = %q, want %q", cfg.Consumer.OCRDataDir, want)
	}
}
