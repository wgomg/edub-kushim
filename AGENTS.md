...
- Version bumps never land on `dev`. After a PR merge, local refs don't need to move — GitHub merges server-side; if you want local `master` to match, `git fetch origin && git branch -f master origin/master`. Merge `origin/master` into `dev` only when dev is behind master (`git merge-base --is-ancestor dev origin/master` fails, e.g. after direct bypass pushes) — in the normal PR-only flow there is nothing to do.
